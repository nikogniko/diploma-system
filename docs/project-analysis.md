...
Статусний потік: кожна фактична зміна статусу додає `ApplicationStatusHistory`; запит з тим самим статусом повертає поточний запис без дублювання історії. Студент може самостійно встановити лише `WITHDRAWN` для власного відгуку. При `HIRED` сервіс перевіряє відсутність іншого найнятого application та в одній transaction закриває vacancy (`CLOSED`, `closedAt`, `closeReason=HIRED`). Автоматичного відхилення інших кандидатів немає.
...
`MatchingScoreService` наразі є розширюваним skeleton: формує factual explanation, matched/missing requirement groups та `baseRequirementsPercent` для ручної перевірки. Фінальний `score` залишається `null`, доки не буде затверджено прозору формулу; відповідно фейковий `matchScore` не записується. Snapshot preview може зберігатися в `Application.matchDetails`.
