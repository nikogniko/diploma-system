...
- `VacanciesPage` збирає стан пошуку, фільтрів, сортування та пагінації і викликає `GET /api/vacancies/search`. Старий `GET /api/vacancies/student` залишений як сумісний alias.
...
- Без текстового `search` relevance не обчислюється: `sortBy=relevance` нормалізується до `updatedAt desc`. `title` і `closingDate` прибрані з основного sorting UI, бо вони менш корисні для студентського каталогу, ніж релевантність, свіжість і зарплата.
