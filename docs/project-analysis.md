...
HR переглядає те саме оформлене резюме кандидата з application card через захищений endpoint. Контакти не входять у загальний список applications: для `PUBLIC` вони відкриваються після подання відгуку, для `APPLIED_ONLY` - лише після оферу, а `HIDDEN` і далі блокує подання відгуку.
...
У спільній сторінці вакансії кнопка `Відгукнутися` виконує eligibility і створення; feedback має dismiss-action. У вкладці студента `Відгуки на вакансії` відображаються власні applications, статусні фільтри, withdraw для нетермінальних статусів і розгортання аналізу. У HR-панелі конкретної вакансії є process-step filter, сортування за `matchScore`, `baseRequirementsPercent` або датою та expandable analysis.
